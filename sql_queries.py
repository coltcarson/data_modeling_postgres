###################################################
# Drops Table(s) In Postgres Database If They Exist
###################################################
...
############################################################
# Creates Table(s) in Postgres Database If They Do Not Exist
############################################################

songplay_table_create = ("""CREATE TABLE IF NOT EXISTS songplays (songplay_id SERIAL PRIMARY KEY, start_time timestamp REFERENCES time(start_time), user_id int REFERENCES users(user_id), level varchar, song_id varchar REFERENCES songs(song_id), artist_id varchar REFERENCES artists(artist_id), session_id int, location varchar, user_agent varchar);""")
...
#####################################
# Inserts Rows Into Postgres Database
#####################################
...
##################################
# Finds Songs in Postgres Database
##################################

song_select = ("""SELECT songs.song_id, artists.artist_id FROM songs JOIN artists ON songs.artist_id=artists.artist_id WHERE title = (%s) AND name = (%s) AND duration = (%s);""")

#######################################
# List of Queries For Postgres Database
#######################################
